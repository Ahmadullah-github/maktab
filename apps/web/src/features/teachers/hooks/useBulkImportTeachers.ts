    ...
    reader.onload = (e) => {
      void (async () => {
        try {
          ...
          if (!(data instanceof ArrayBuffer)) throw new Error('Invalid spreadsheet data');
        ...
        sheet.eachRow((row, rowNumber) => {
          ...
          jsonData.push(Object.fromEntries(headers.map((header, index) => [header, values[index] ?? ''])));
        });
...
export async function generateExcelTemplate(): Promise<void> {
  ...
  // Combine all data
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('معلمین', { views: [{ rightToLeft: true }] });
  worksheet.addRows([headers, ...emptyRows]);
  worksheet.columns = [25, 18, 30, 20, 20].map((width) => ({ width }));
  ...
  const instructionsSheet = workbook.addWorksheet('راهنما', { views: [{ rightToLeft: true }] });
  ...
  anchor.href = url;
  anchor.download = 'قالب_وارد_کردن_معلمین.xlsx';
  anchor.click();
