...
function buildErrorFromLastRun(lastRun: SolverStatus['lastRun']): ScheduleGenerationError | null {
...
function parseGenerationError(error: unknown): ScheduleGenerationError | null {
...
export function useGenerateSchedule(): UseGenerateScheduleReturn {
  ...
  const [error, setError] = useState<ScheduleGenerationError | null>(null);
  ...
  const getBlockedReason = useCallback((): string | null => {
    ...
    const mode = licenseStatus?.state;
    if (mode === 'expired') {
  ...
  }, [canGenerateLicense, licenseStatus?.state]);
  ...
  const generate = useCallback(
    ...
    [canGenerateLicense, getBlockedReason, mutation]
  );
