...
export function useExportSchedule(): UseExportScheduleReturn {
  ...
  const exportMutation = useMutation({
    mutationFn: async (request: ExportRequest) => {
      if (request.format === 'pdf' && window.maktab) {
        const result = await window.maktab.documents.savePdf({ suggestedName: 'maktab-timetable.pdf' });
        if (!result.ok) throw new Error(result.error.message);
        if (!result.value.canceled) showSuccessNotification(false);
        return result.value;
      }
