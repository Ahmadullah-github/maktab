...
class DecompositionConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="Enable automatic problem decomposition for large problems"
    )
    threshold: int = Field(
        default=200, ge=50, le=1000, description="Lesson count threshold to trigger decomposition"
    )
    large_threshold: int = Field(
        default=250, ge=100, le=1500, description="Threshold for 'large' problem classification"
    )
    ...
    max_cluster_size: int = Field(
        default=150, ge=50, le=500, description="Maximum lessons per cluster during decomposition"
    )
...
class StrategyConfig(BaseModel):
    ...
    workers: int = Field(
        default=4, ge=1, le=64, description="Number of parallel workers for CP-SAT solver"
    )
    max_time_seconds: int = Field(
        default=600, ge=10, le=7200, description="Maximum solve time in seconds"
    )
    probing_level: int = Field(
        default=0, ge=0, le=3, description="CP-SAT probing level (0=auto, 1-3=increasing intensity)"
    )
    linearization_level: int = Field(
        default=0, ge=0, le=2, description="CP-SAT linearization level (0=auto, 1-2=increasing)"
    )
...
class MemoryConfig(BaseModel):
    ...
    max_memory_mb: int = Field(
        default=4096, ge=512, le=32768, description="Maximum memory usage in megabytes"
    )
    warning_threshold: float = Field(
        default=0.8, ge=0.5, le=0.95, description="Memory usage ratio to trigger warning"
    )
    gc_threshold: float = Field(
        default=0.9, ge=0.6, le=0.99, description="Memory usage ratio to trigger garbage collection"
    )
...
class CheckpointConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Enable checkpoint saving during solving")
    directory: str = Field(
        default="./checkpoints", description="Directory to store checkpoint files"
    )
    save_interval_seconds: int = Field(
        default=60, ge=10, le=600, description="Interval between checkpoint saves"
    )
...
class SolverConfig(BaseModel):
    ...
    decomposition: DecompositionConfig = Field(
        default_factory=DecompositionConfig, description="Decomposition settings"
    )
    ...
    memory: MemoryConfig = Field(
        default_factory=MemoryConfig, description="Memory management settings"
    )
    checkpoints: CheckpointConfig = Field(
        default_factory=CheckpointConfig, description="Checkpoint/resume settings"
    )
