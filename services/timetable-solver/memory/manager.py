...
class MemoryError(Exception):
    ...
    def __init__(
        ...
        full_message = f"{message} (current: {current_mb:.1f}MB, limit: {limit_mb}MB)"
        if diagnostics:
...
class MemoryManager:
    ...
    def __init__(
        self, max_memory_mb: int = 4096, warning_threshold: float = 0.8, gc_threshold: float = 0.9
    ):
        """
    ...
    def get_diagnostics(self) -> dict[str, Any]:
        ...
        pool_stats = {name: len(pool) for name, pool in self._variable_pools.items()}

        return {
            "current_memory_mb": round(current_mb, 2),
            "max_memory_mb": self.max_memory_mb,
            "peak_memory_mb": round(self._peak_memory_mb, 2),
            "memory_ratio": round(current_mb / self.max_memory_mb, 3)
