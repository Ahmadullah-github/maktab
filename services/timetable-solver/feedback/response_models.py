...
class SolverErrorDetail(BaseModel):
    ...
    affected_entities: list[AffectedEntity] = Field(
        default_factory=list, description="List of entities affected by this error"
    )
    context: dict[str, Any] = Field(
        default_factory=dict, description="Additional context data for UI rendering"
    )
...
class QualityBreakdown(BaseModel):
    ...
    Attributes:
        ...
        afternoon_difficult_subjects: Difficult afternoon subjects and their penalty/details.
        same_day_subject_repetition: Same-day subject repetitions and their penalty/details.
...
class Suggestion(BaseModel):
    ...
    affected_entities: list[AffectedEntity] = Field(
        default_factory=list, description="Entities affected by this suggestion"
    )
    expected_improvement: int = Field(..., ge=0, description="Estimated score increase (0-100)")
...
class QualityScore(BaseModel):
    ...
    overall: int = Field(..., ge=0, le=100, description="Overall quality score (0-100)")
    breakdown: QualityBreakdown = Field(
        default_factory=QualityBreakdown, description="Detailed breakdown of quality components"
    )
...
class SolverResponseMetadata(BaseModel):
    ...
    solve_time_seconds: float | None = Field(None, description="Time taken to solve in seconds")
    strategy_selected: str | None = Field(
        None, description='Strategy used: "fast", "balanced", or "thorough"'
    )
    strategy_reason: str | None = Field(None, description="Explanation for strategy selection")
    strategy_overridden: bool | None = Field(
        None, description="Whether user explicitly specified the strategy"
    )
    total_lessons: int | None = Field(None, description="Total number of lessons to schedule")
    optimization_preferences_revision: int | None = Field(default=None, ge=1)
    enabled_objectives: list[str] = Field(default_factory=list)
    # Afghanistan-specific metadata (Requirements: 1.5, 4.4)
    ramadan_mode_enabled: bool | None = Field(None, description="Whether Ramadan mode was active")
    ramadan_period_duration: int | None = Field(
        None, description="Period duration in minutes used in Ramadan mode"
    )
    low_resource_mode: bool | None = Field(None, description="Whether low-resource mode was active")
    max_workers: int | None = Field(
        None, description="Max worker threads used in low-resource mode"
    )
    max_memory_mb: int | None = Field(
        None, description="Max memory in MB used in low-resource mode"
    )
...
class SolverResponse(BaseModel):
    ...
    data: dict[str, Any] | None = Field(None, description="Timetable data when successful")
    errors: list[SolverErrorDetail] = Field(
        default_factory=list, description="List of errors that blocked generation"
    )
    warnings: list[SolverErrorDetail] = Field(
        default_factory=list, description="List of warnings (non-blocking issues)"
    )
    quality_score: QualityScore | None = Field(
        None, description="Quality assessment of the generated timetable"
    )
    metadata: SolverResponseMetadata = Field(
        default_factory=SolverResponseMetadata, description="Execution metadata"
    )
