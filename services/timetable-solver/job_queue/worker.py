...
class ResultStore(Protocol):
    ...
    def get_result(self, job_id: str) -> SolverResult | None:
            ...
            while self.running:
                try:
                    ...
                    job_data = self.redis_client.blpop(
                        self.queue_name, timeout=int(self.poll_interval)
                    )
    ...
    def process_job(self, job: SolverJob) -> SolverResult:
        ...
        try:
            ...
            if job.config:
                ...
                # Apply the simple top-level merge expected by the job contract.
            ...
            output = solver.solve(time_limit_seconds=job.time_limit_seconds, strategy=job.strategy)
            ...
            if isinstance(output, list):
                ...
                solver_output = SolverOutput(schedule=output, status=SolverStatus.SUCCESS)
            ...
            if job.retry_count < job.max_retries:
                self.logger.info(
                    f"Job {job.job_id}: Scheduling retry {job.retry_count + 1}/{job.max_retries}"
                )
...
def create_worker_from_config(redis_url: str, config: dict[str, Any] | None = None) -> SolverWorker:
    """
