...
class SolverJob(BaseModel):
    ...
    config: dict[str, Any] | None = Field(
        default=None, description="Solver configuration overrides"
    )
    time_limit_seconds: int = Field(default=600, description="Time limit for solving")
    strategy: str = Field(
        default="balanced", description="Solving strategy (fast/balanced/thorough)"
    )
...
class SolverResult(BaseModel):
    ...
    output: SolverOutput | None = Field(default=None, description="Solver output if successful")
    error: str | None = Field(default=None, description="Error message if failed")
    processing_time_seconds: float | None = Field(default=None, description="Time taken to process")
    completed_at: float = Field(default_factory=time.time, description="Completion timestamp")
    retry_count: int = Field(default=0, description="Number of retries attempted")
    worker_id: str | None = Field(default=None, description="ID of worker that processed the job")
