...
type FixedRoomClass = ClassGroup & {
  subjectRequirements?: Array<{ periodsPerWeek?: number }>;
};
...
export async function validateFixedRoomAssignments(
  classes: FixedRoomClass[],
  rooms: Room[]
): Promise<FeasibilityWarning[]> {
  const warnings: FeasibilityWarning[] = [];
  
  // Group classes by their fixed room
  const roomAssignments = new Map<number, FixedRoomClass[]>();
