...
class TeacherConstraintTests(unittest.TestCase):
    ...
    def test_pre_solve_does_not_reject_feasible_asymmetric_capacity(self):
        ...
        data = TimetableData(**payload(periods=10, days=["Saturday", "Sunday"], teachers=teachers))
    ...
    def test_fixed_split_assignment_uses_exact_period_counts(self):
        ...
        self.assertEqual(result["status"], "success")
        counts = Counter(lesson["teacherIds"][0] for lesson in result["data"]["schedule"])
    ...
    def test_subject_shape_constraints_still_apply(self):
        ...
        self.assertTrue(
            all(
                ...
                for left, right in zip(sorted(periods), sorted(periods)[1:], strict=False)
            )
        )
    ...
    def test_time_preference_is_optimized(self):
        ...
        data["teachers"].append(filler)
        data["subjects"].extend(
            {"id": subject_id, "name": subject_id} for subject_id in filler_subjects
        )
        data["classes"][0]["subjectRequirements"] = {
        ...
        self.assertEqual(result["status"], "success")
        target = next(
            lesson for lesson in result["data"]["schedule"] if lesson["subjectId"] == "subject"
        )
