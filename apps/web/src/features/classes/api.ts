...
export const classesApi = {
  ...
  async bulkApplyCurriculum(
    ...
    apiLogger.request('POST', '/curriculum/plan/preview', { classIds });

    try {
      const plan = await schoolCurriculumApi.getPlan();
      const preview = await schoolCurriculumApi.preview({
        revision: plan.revision,
        changedGrades: [],
        synchronizeClassIds: classIds,
        proposedClasses: [],
      });
      if (preview.blockers.length > 0) throw new Error(preview.blockers[0].message);
      const applied = await schoolCurriculumApi.apply(preview.previewToken, false);
      const response = {
        updated: applied.synchronizedClassIds.length,
        skipped: 0,
        failed: 0,
        details: preview.affectedClasses.map((classGroup) => ({
      ...
      apiLogger.response('POST', '/curriculum/plan/apply', 200, {
      ...
      apiLogger.error('POST', '/curriculum/plan/apply', error);
      throw error;
    }
  },
};
