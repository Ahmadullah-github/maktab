 ...
 * canonical teaching assignments into the format required by the
...
interface CanonicalSwapAssignment {
  teacherId: number;
  classId: number;
  subjectId: number;
    ...
    const [timetable, teachers, subjects, rooms, classes, assignments] = await Promise.all([
      ...
      this.dataSource.query(`
        SELECT a.teacher_id AS teacherId,
         ...
         WHERE a.is_deleted = 0 AND r.is_deleted = 0
      `) as Promise<CanonicalSwapAssignment[]>,
    ]);
