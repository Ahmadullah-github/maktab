...
export class ClassService {
  ...
  private async populateFromCurriculum(
    ...
    const plan = await manager.getRepository(SchoolCurriculumPlan).findOne({
      where: { schoolId: schoolId === null ? IsNull() : schoolId },
    });
    if (!plan) return [];
    const items = await manager.getRepository(SchoolCurriculumItem).find({
      where: { planId: plan.id, grade },
      order: { position: 'ASC' },
    });
    const requirements: SubjectRequirement[] = [];
    for (const item of items) {
      const subject = await this.subjectRepository.findByCurriculumItemId(item.id);
      if (!subject) {
        throw new Error(`Curriculum item ${item.id} has no active catalog subject`);
      }
      requirements.push({
