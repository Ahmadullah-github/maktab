...
async function startServer(databasePath) {
  ...
  return {
    baseUrl: `http://${ready.host}:${ready.port}/local-api/v1`,
    child,
    output: () => output,
  };
}
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal((await staleConfigUpdate.json()).error.code, 'CONFIG_REVISION_CONFLICT');
      ...
      const curriculumPlanResponse = await apiRequest(server.baseUrl, '/curriculum/plan');
      assert.equal(curriculumPlanResponse.status, 200);
      const curriculumPlan = await curriculumPlanResponse.json();
      assert.equal(curriculumPlan.revision, 0);
      assert.equal(curriculumPlan.grades.every((grade) => grade.items.length === 0), true);
      const curriculumTemplate = await (
        await apiRequest(server.baseUrl, '/curriculum/templates/afghanistan')
      ).json();
      assert.ok(curriculumTemplate.grades.find((grade) => grade.grade === 6).items.length > 0);

      const curriculumPreviewResponse = await apiRequest(
        server.baseUrl,
        '/curriculum/plan/preview',
        {
          method: 'POST',
          body: JSON.stringify({
            revision: curriculumPlan.revision,
            changedGrades: [{
              grade: 6,
              items: [{ name: 'مضمون مکتب', nameEn: 'School Subject', code: 'SCH6', weeklyPeriods: 2 }],
            }],
            synchronizeClassIds: [],
            proposedClasses: [],
          }),
      ...
      const curriculumPreviewText = await curriculumPreviewResponse.text();
      assert.equal(curriculumPreviewResponse.status, 200, curriculumPreviewText);
      const curriculumPreview = JSON.parse(curriculumPreviewText);
      assert.deepEqual(curriculumPreview.blockers, []);
      const curriculumApplyResponse = await apiRequest(server.baseUrl, '/curriculum/plan/apply', {
        method: 'POST',
        body: JSON.stringify({
          previewToken: curriculumPreview.previewToken,
          confirmAssignmentRemoval: false,
        }),
      });
      assert.equal(curriculumApplyResponse.status, 200, await curriculumApplyResponse.text());
      const reusedPreview = await apiRequest(server.baseUrl, '/curriculum/plan/apply', {
        method: 'POST',
        body: JSON.stringify({
          previewToken: curriculumPreview.previewToken,
          confirmAssignmentRemoval: false,
      ...
      assert.equal(reusedPreview.status, 409);
      assert.equal((await reusedPreview.json()).error.code, 'PREVIEW_EXPIRED');

      const staleCurriculumPreview = await apiRequest(
        server.baseUrl,
        '/curriculum/plan/preview',
        {
          method: 'POST',
      ...
      assert.equal(staleCurriculumPreview.status, 409);
      assert.equal((await staleCurriculumPreview.json()).error.code, 'CURRICULUM_REVISION_STALE');
      ...
      assert.equal(calendarConflict.error.code, 'AVAILABILITY_OUT_OF_BOUNDS');
      assert.deepEqual(calendarConflict.error.details.conflicts[0], {
      ...
      assert.equal(gradeBandConflictBody.error.code, 'GRADE_BAND_IN_USE');
      assert.equal(gradeBandConflictBody.error.details.band, 'primary');
      assert.equal(gradeBandConflictBody.error.details.classCount, 1);
      ...
      assert.ok(
        fs
          .readdirSync(path.join(temporaryDirectory, 'recovery'))
          .some((name) => name.startsWith('timetable.db.migration-')),
      ...
      assert.equal(
        database.pragma('table_info(teacher)').some((column) => column.name === 'classAssignments'),
        false
      );
      assert.equal(database.pragma('integrity_check', { simple: true }), 'ok');
      assert.deepEqual(database.pragma('foreign_key_check'), []);
...
test('room contracts are dynamic, atomic, normalized, and recoverable', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    assert.equal((await duplicateRoom.json()).error.code, 'ROOM_NAME_CONFLICT');
    ...
    assert.equal((await blockedTypeDelete.json()).error.code, 'ROOM_TYPE_DELETE_BLOCKED');
    ...
    assert.equal((await atomicDelete.json()).error.code, 'ROOM_DELETE_BLOCKED');
    assert.deepEqual(
    ...
    assert.equal((await mixedScopeWrite.json()).error.code, 'SCHOOL_SCOPE_CONFLICT');
  } finally {
    if (server) await stopServer(server);
    fs.rmSync(temporaryDirectory, { recursive: true, force: true });
  }
...
test('managed assignment migration backfills valid legacy teacher mirrors', async () => {
  ...
  try {
    ...
    await dataSource.query(
      'INSERT INTO subject (name, code, periodsPerWeek) VALUES (?, ?, ?)',
      ['Legacy Mathematics', 'LEG-MATH', 3]
    );
    const [{ id: subjectId }] = await dataSource.query('SELECT last_insert_rowid() AS id');
    ...
    await dataSource.query('UPDATE class_group SET subjectRequirements = ? WHERE id = ?', [
      JSON.stringify([{ subjectId: subject.id, periodsPerWeek: 3, teacherId: null }]),
      classGroup.id,
    ]);
    await dataSource.query(
      'UPDATE teacher SET primarySubjectIds = ?, allowedSubjectIds = ?, classAssignments = ? WHERE id = ?',
      [
        JSON.stringify([subject.id]),
        '[]',
        JSON.stringify([{ subjectId: subject.id, classIds: [classGroup.id] }]),
        teacher.id,
      ]
    );
    await dataSource.destroy();
    ...
    await dataSource.query(
      'UPDATE teaching_assignment SET assigned_periods_per_week = assigned_periods_per_week + 1'
    );
    await dataSource.destroy();
...
test('canonical assignment batches are atomic, versioned, policy checked, and stale schedules', async () => {
  ...
  try {
    ...
    assert.equal(storageAudit.isConsistent, true, JSON.stringify(storageAudit.issues));
    assert.equal(
      await dataSource.getRepository(TeachingAssignment).countBy({ isDeleted: false }),
      2
    );
...
test(
  ...
  async () => {
    ...
    try {
      ...
      database.exec(
        `ALTER TABLE teacher ADD COLUMN primarySubjectIds text NOT NULL DEFAULT '[]'`
      );
      database.exec(
        `ALTER TABLE teacher ADD COLUMN allowedSubjectIds text NOT NULL DEFAULT '[]'`
      );
      database.exec(
        `ALTER TABLE teacher ADD COLUMN classAssignments text NOT NULL DEFAULT '[]'`
      );
      database.exec(
      ...
      database.prepare('DELETE FROM migrations WHERE timestamp >= ?').run(1783900000000);
      database
        .prepare(
      ...
      const repairedTeacher = database
        .prepare('SELECT unavailable, preferredRoomIds FROM teacher')
        .get();
