...
class SwapValidator:
    ...
    def _get_lesson_at_slot(self, class_id: str, day: str, period: int) -> Lesson | None:
        """Find lesson at specific slot."""
        key = f"{class_id}_{day}_{period}"
        return self.slot_index.get(key)

    def _is_same_slot(self, lesson: Lesson, class_id: str, day: str, period: int) -> bool:
        """Check if lesson is at the specified slot."""
        return lesson.classId == class_id and lesson.day == day and lesson.periodIndex == period
    ...
    def _day_period_limit(self, day: str) -> int | None:
    ...
    @staticmethod
    def _matches_lesson(first: Lesson, second: Lesson | None) -> bool:
        if second is None:
            return False
        return (
    ...
    def _check_full_move_constraints(
        self, source: Lesson, target: Lesson | None, request: SwapRequest
        ...
        for lesson, destination_day, destination_period in moves:
            ...
            for teacher_id in lesson.teacherIds:
                ...
                if not teacher:
                    ...
                    teacher_name = teacher.get("fullName", teacher_id)
                    violations.append(
                        ConstraintViolation(
                            type="TEACHER_UNAVAILABLE",
                            severity="hard",
                            message=(
                                f"Teacher {teacher_name} is unavailable for the full moved lesson"
                            ),
                            message_farsi=(
                                f"استاد {teacher_name} برای تمام مدت درس جابه‌جا شده در دسترس نیست"
                            ),
                            details={
            ...
            if not room:
                ...
                if any(
                    self._slot_is_unavailable(room, destination_day, destination_period + offset)
                    for offset in range(duration)
                ):
                    violations.append(
                        ConstraintViolation(
    ...
    def validate_swap(self, swap_request: SwapRequest) -> SwapResolution:
        ...
        if not source_lesson:
            errors.append(
                ConstraintViolation(
                    ...
                    severity="hard",
                    message="No lesson found at source slot",
        ...
        errors.extend(self._check_full_move_constraints(source_lesson, target_lesson, swap_request))
    ...
    def _check_teacher_availability(
        ...
        if source_teacher and not self._is_available(
            ...
            source_teacher_name = source_teacher.get("fullName", source.teacherId)
            violations.append(
                ConstraintViolation(
                    type="TEACHER_UNAVAILABLE",
                    severity="hard",
                    message=f"Teacher {source_teacher_name} is unavailable at target time",
                    message_farsi=f"استاد {source_teacher_name} در زمان مقصد در دسترس نیست",
                    details={
        ...
        if target:
            ...
            if target_teacher and not self._is_available(
                ...
                target_teacher_name = target_teacher.get("fullName", target.teacherId)
                violations.append(
                    ConstraintViolation(
                        type="TEACHER_UNAVAILABLE",
                        severity="hard",
                        message=f"Teacher {target_teacher_name} is unavailable at source time",
                        message_farsi=f"استاد {target_teacher_name} در زمان مبدأ در دسترس نیست",
                        details={
        ...
        if source_teacher:
            for lesson in self.assignments:
                ...
                if (
                    ...
                    teacher_name = source_teacher.get("fullName", source.teacherId)
                    violations.append(
                        ConstraintViolation(
                            type="TEACHER_CONFLICT",
                            severity="hard",
                            message=(f"Teacher {teacher_name} already has a lesson at target time"),
                            message_farsi=f"استاد {teacher_name} در زمان مقصد درس دیگری دارد",
                            details={
                                "teacherId": source.teacherId,
                                "teacherName": source_teacher.get("fullName"),
            ...
            if target_teacher:
                for lesson in self.assignments:
                    ...
                    if (
                        ...
                        teacher_name = target_teacher.get("fullName", target.teacherId)
                        violations.append(
                            ConstraintViolation(
                                type="TEACHER_CONFLICT",
                                severity="hard",
                                message=(
                                    f"Teacher {teacher_name} already has a lesson at source time"
                                ),
                                message_farsi=f"استاد {teacher_name} در زمان مبدأ درس دیگری دارد",
                                details={
                                    "teacherId": target.teacherId,
                                    "teacherName": target_teacher.get("fullName"),
                                    "conflictingClass": lesson.classId,
        ...
        if source.roomId:
            for lesson in self.assignments:
                ...
                if (
                    ...
                    room_name = room.get("name", source.roomId)
                    violations.append(
                        ConstraintViolation(
                            type="ROOM_CONFLICT",
                            severity="hard",
                            message=f"Room {room_name} is already occupied at target time",
                            message_farsi=f"اتاق {room_name} در زمان مقصد اشغال است",
                            details={
        ...
        if target and target.roomId:
            for lesson in self.assignments:
                ...
                if (
                    ...
                    room_name = room.get("name", target.roomId)
                    violations.append(
                        ConstraintViolation(
                            type="ROOM_CONFLICT",
                            severity="hard",
                            message=f"Room {room_name} is already occupied at source time",
                            message_farsi=f"اتاق {room_name} در زمان مبدأ اشغال است",
                            details={
    ...
    def _check_room_type_requirements(
        ...
        if required_room_type and target and target.roomId:
            ...
            if target_room.get("type") != required_room_type:
                subject_name = source_subject.get("name", source.subjectId)
                actual_type = target_room.get("type")
                violations.append(
                    ConstraintViolation(
                        type="ROOM_TYPE_MISMATCH",
                        severity="hard",
                        message=(
                            f"Subject {subject_name} requires {required_room_type} "
                            f"but target room is {actual_type}"
                        ),
                        message_farsi=(
                            f"درس {subject_name} نیاز به {required_room_type} دارد "
                            f"اما اتاق مقصد {actual_type} است"
                        ),
                        details={
        ...
        # Check the target subject's room requirement at the source slot.
        if target:
            ...
            if required_room_type and source.roomId:
                ...
                if source_room.get("type") != required_room_type:
                    subject_name = target_subject.get("name", target.subjectId)
                    actual_type = source_room.get("type")
                    violations.append(
                        ConstraintViolation(
                            type="ROOM_TYPE_MISMATCH",
                            severity="hard",
                            message=(
                                f"Subject {subject_name} requires {required_room_type} "
                                f"but source room is {actual_type}"
                            ),
                            message_farsi=(
                                f"درس {subject_name} نیاز به {required_room_type} دارد "
                                f"اما اتاق مبدأ {actual_type} است"
                            ),
                            details={
            ...
            if self._is_afternoon_period(request.target_slot.period):
                ...
                violations.append(
                    ConstraintViolation(
                        ...
                        severity="soft",
                        message=f"Difficult subject {subject_name} scheduled in afternoon",
                        message_farsi=(f"درس سخت {subject_name} در بعد از ظهر برنامه‌ریزی شده است"),
                        details={
                            "subjectId": source.subjectId,
                            "subjectName": source_subject.get("name"),
                            "period": request.target_slot.period,
                ...
                if self._is_afternoon_period(request.source_slot.period):
                    ...
                    violations.append(
                        ConstraintViolation(
                            ...
                            message=f"Difficult subject {subject_name} scheduled in afternoon",
                            message_farsi=(
                                f"درس سخت {subject_name} در بعد از ظهر برنامه‌ریزی شده است"
                            ),
                            details={
                                "subjectId": target.subjectId,
                                "subjectName": target_subject.get("name"),
    ...
    def _check_teacher_preferences(
        ...
        for teacher_id in source.teacherIds:
            violations.extend(
                self._teacher_preference_violations(teacher_id, request.target_slot.period)
            )
        ...
        if target:
            for teacher_id in target.teacherIds:
                violations.extend(
                    self._teacher_preference_violations(teacher_id, request.source_slot.period)
                )
    ...
    def _teacher_preference_violations(
        ...
        violates = (preference == "Morning" and self._is_afternoon_period(destination_period)) or (
            preference == "Afternoon" and not self._is_afternoon_period(destination_period)
        )
        if not violates:
            return []
        ...
        return [
            ConstraintViolation(
                ...
                severity="soft",
                message=(
                    f"Teacher {teacher_name} prefers {str(preference).lower()} "
                    f"but would be scheduled in {destination}"
                ),
                message_farsi=f"ترجیح زمانی استاد {teacher_name} در جابه‌جایی رعایت نمی‌شود",
                details={
                    "teacherId": teacher_id,
