...
log = get_logger("solver.decomposition")
...
class DecompositionSolver:
    ...
    def __init__(self, input_data, solver_class):
        ...
        if isinstance(input_data, dict):
            from solver import TimetableData
        ...
        log.info(
            "DecompositionSolver initialized",
    ...
    def should_decompose(self) -> bool:
        ...
        if self.num_requests < self.DECOMPOSITION_THRESHOLD:
            log.info(
                "Problem too small for decomposition",
                num_requests=self.num_requests,
        ...
        if self.num_requests >= self.LARGE_PROBLEM_THRESHOLD:
            log.info(
                "Problem large enough for decomposition",
                num_requests=self.num_requests,
        ...
        if shared_teacher_ratio < 0.3:  # Less than 30% sharing
            log.info(
                "Medium problem with low teacher sharing - decompose",
                num_requests=self.num_requests,
            ...
            log.info(
                "Medium problem with high teacher sharing - regular solver",
                num_requests=self.num_requests,
                shared_teacher_ratio=f"{shared_teacher_ratio:.2%}",
                decision="use_regular_solver",
            )
    ...
    def _analyze_teacher_sharing(self) -> float:
        ...
        multi_class_teachers = sum(
            1 for classes in teacher_classes.values() if len(classes) > 1
        )
    ...
    def _check_grade_independence(self) -> bool:
        ...
        multi_grade_teachers = sum(
            1 for grades in teacher_grades.values() if len(grades) > 1
        )

        # If more than 20% of teachers teach multiple grades, not independent
        independence_threshold = 0.2
        is_independent = (
            multi_grade_teachers / len(teacher_grades) < independence_threshold
        )

        log.info(
            "Grade independence check",
    ...
    def solve(self, **solver_kwargs) -> List[Dict[str, Any]]:
        ...
        if self.strategy == DecompositionStrategy.NONE:
            ...
            input_for_solver = (
                self.input_dict if self.input_dict is not None else self.data
            )
        ...
        try:
            ...
            if self.strategy == DecompositionStrategy.CLASS_CLUSTERING:
                result = self._solve_with_clustering(**solver_kwargs)
            elif self.strategy == DecompositionStrategy.GRADE_LEVEL:
                result = self._solve_by_grade_level(**solver_kwargs)
            elif self.strategy == DecompositionStrategy.TWO_PHASE:
                result = self._solve_two_phase(**solver_kwargs)
            else:
                raise ValueError(f"Unknown strategy: {self.strategy}")

            # Check if the result is an error - don't fall back, just return the error
            if result and len(result) > 0 and result[0].get("error"):
                log.error(
                    "Decomposition returned error, not falling back",
            ...
            log.error(
                "Decomposition failed with exception",
                error=str(e),
                exc_info=True,
            )
            # Return error instead of falling back (which would fail the same way)
            return [{"error": str(e), "status": "DECOMPOSITION_ERROR"}]
    ...
    def _solve_with_clustering(self, **solver_kwargs) -> List[Dict[str, Any]]:
        ...
        # Collect errors from failed clusters
        cluster_errors = []
        ...
        for i, cluster in enumerate(clusters):
            log.info(
                f"Solving cluster {i+1}/{len(clusters)}",
            ...
            try:
                ...
                if not solution or (
                    ...
                    and len(solution) > 0
                    and "error" in solution[0]
                ):
                    error_msg = solution[0].get("error") if solution else "No solution"
                    log.error(
                        f"Cluster {i+1} failed to solve",
                        error=error_msg,
                    )
                    ...
                    self.sub_solutions.append(
                        {"cluster_id": i, "solution": solution, "cluster": cluster}
                    )
            except RuntimeError as e:
                error_msg = str(e)
                log.error(f"Runtime error during solving", error=error_msg)
                cluster_errors.append(
                    {
                        "cluster": i + 1,
                        "classes": cluster.get("classes", []),
        ...
        if len(cluster_errors) == len(clusters):
            # Extract the most informative error message
            first_error = (
                cluster_errors[0]["error"] if cluster_errors else "Unknown error"
            )
            return [
                {
                    "error": first_error,
                    "status": "ALL_CLUSTERS_FAILED",
                    "cluster_errors": cluster_errors,
                }
            ]

        # If some clusters failed, log warning but continue with partial solution
        if cluster_errors:
            log.warning(
                f"{len(cluster_errors)} of {len(clusters)} clusters failed",
                failed_clusters=[e["cluster"] for e in cluster_errors],
            )

        # Merge solutions
        merger = SolutionMerger(self.data)
        merged = merger.merge(self.sub_solutions)

        # If no lessons were merged, return error with details
        if not merged or len(merged) == 0:
            first_error = (
                cluster_errors[0]["error"] if cluster_errors else "No solution found"
            )
            return [
                {
                    "error": first_error,
    ...
    def _solve_by_grade_level(self, **solver_kwargs) -> List[Dict[str, Any]]:
        ...
        for grade_idx, (grade_level, grade_classes) in enumerate(grade_groups.items()):
            log.info(
                f"Solving grade level {grade_level}",
            ...
            # Filter teachers
            sub_data.teachers = [
                t for t in sub_data.teachers if t.id in teachers_needed
            ]
            ...
            if sub_data.fixedLessons:
                sub_data.fixedLessons = [
                    lesson
                    for lesson in sub_data.fixedLessons
            ...
            try:
                ...
                if not solution or (
                    ...
                    log.error(
                        f"Grade level {grade_level} failed to solve",
                ...
                log.error(
                    f"Exception solving grade level {grade_level}: {e}", exc_info=True
                )
    ...
    def _solve_two_phase(self, **solver_kwargs) -> List[Dict[str, Any]]:
        ...
        complete_solution = self._phase2_assign_resources(
            time_assignments, **solver_kwargs
        )
    ...
    def _phase1_assign_time_slots(self, **solver_kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            phase1_kwargs["time_limit_seconds"] = (
                solver_kwargs.get("time_limit_seconds", 600) // 2
            )
    ...
    def _phase2_assign_resources(
        self, time_assignments: List[Dict[str, Any]], **solver_kwargs
    ) -> List[Dict[str, Any]]:
        """
