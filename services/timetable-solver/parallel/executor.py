...
class ParallelClusterExecutor:
    ...
    def __init__(self, max_workers: int | None = None, config: Any | None = None):
        """
    ...
    def solve_clusters(
        ...
        cluster_data_builder: Callable[[dict[str, Any]], dict[str, Any]] | None = None,
        **solver_kwargs,
    ) -> list[ClusterResult]:
        """
            ...
            cluster_work_items.append(
                {
                    ...
                    "cluster_metadata": {
                        k: v for k, v in cluster.items() if k not in ("sub_problem_data",)
                    },
                }
            )
            ...
            for future in as_completed(future_to_cluster):
                ...
                try:
                    ...
                    if result.success:
                        ...
                        log.warning(
                            "Cluster solving failed", cluster_id=cluster_id, error=result.error
                        )
    ...
    def merge_solutions(self, results: list[ClusterResult], data: Any) -> list[dict[str, Any]]:
        """
        ...
        for result in results:
            if result.success:
                ...
                failed_clusters.append({"cluster_id": result.cluster_id, "error": result.error})
