...
import type { SolverInput } from '../../services/solverDataTransformer.service';
import {
...
function countConstraints(input: SolverInput): number {
  const unavailable = [...input.teachers, ...input.classes, ...input.rooms].reduce(
    (total, entity) => total + (Array.isArray(entity.unavailable) ? entity.unavailable.length : 0),
    0
  );
  const requirements = input.classes.reduce((total, classGroup) => {
    const rows = classGroup.subjectRequirements ?? classGroup.subject_requirements ?? [];
    return total + (Array.isArray(rows) ? rows.length : 0);
  }, 0);
  return unavailable + requirements + (input.fixedLessons?.length ?? 0) +
    (input.fixedTeacherAssignments?.length ?? 0) + (input.schoolEvents?.length ?? 0);
}

function hashSolverInput(input: SolverInput): string {
...
export async function handleGenerate(
  ...
  const correlationId = req.requestContext?.requestId ?? crypto.randomUUID();
  let evidence: SolverRunEvidence = {
    solverVersion: solverService.getSolverVersion(),
    inputSha256: crypto.createHash('sha256').update('{}').digest('hex'),
    constraintCount: 0,
    durationMs: 0,
    outcome: 'failed',
    correlationId,
  ...
  try {
    ...
    evidence = {
      ...evidence,
      inputSha256: hashSolverInput(solverInput),
      constraintCount: countConstraints(solverInput),
    };
    ...
    if (runStarted && !lastRun) {
      evidence.outcome = err.code === ERROR_CODES.SOLVER_RUNTIME_ERROR ||
        err.code === ERROR_CODES.SOLVER_SPAWN_ERROR || err.code === ERROR_CODES.SOLVER_PARSE_ERROR
        ? 'crashed'
        : 'failed';
      lastRun = createLastRunSummary('failed', {
    ...
    logger.info('Solver run terminal evidence', { ...evidence });
    if (runStarted) {
