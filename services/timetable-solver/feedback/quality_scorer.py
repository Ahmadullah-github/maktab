...
class QualityScorer:
    ...
    def __init__(self, schedule: list[ScheduledLesson], data: TimetableData):
        ...
        self.data = data
        self.days = [
            day.value if isinstance(day, DayOfWeek) else str(day) for day in data.config.daysOfWeek
        ]
    ...
    def _legacy_metric(metric: tuple[int, int, list[AffectedEntity]]) -> dict[str, Any]:
        ...
        return {
            "count": violation,
            "penalty": violation,
            "details": [item.model_dump() for item in affected[:5]],
        }

    def _measure_all(self) -> dict[str, tuple[int, int, list[AffectedEntity]]]:
        metrics: dict[str, tuple[int, int, list[AffectedEntity]]] = {}
        metrics["avoidTeacherGaps"] = self._gap_metric(
            self.teacher_day, "teacher", self.teacher_map
        )
    ...
    def _gap_metric(
        self, grouped, entity_type: str, lookup
    ) -> tuple[int, int, list[AffectedEntity]]:
        ...
        for (entity_id, _day), periods in grouped.items():
            ...
            if len(ordered) < 2:
                continue
            gaps = sum(
                max(0, right - left - 1) for left, right in zip(ordered, ordered[1:], strict=False)
            )
    ...
    def _subject_spread(self):
        violations = opportunities = 0
        affected = []
        for (class_id, _subject_id, _day), periods in self.class_subject_day.items():
    ...
    def _difficult_distribution(self):
        ...
        affected = [
            ...
            for (class_id, _day), subjects in per_day.items()
            if len(subjects) > 1
        ]
        return violations, opportunities, affected
    ...
    def _teacher_time(self):
        ...
        for lesson in self.schedule:
            ...
            for teacher_id in lesson.teacherIds:
                ...
                preference = getattr(
                    getattr(teacher, "timePreference", None),
                    "value",
                    getattr(teacher, "timePreference", None),
                )
                if preference not in ("Morning", "Afternoon"):
                    continue
                opportunities += 1
                wrong = (preference == "Morning" and lesson.periodIndex >= cutoff) or (
                    preference == "Afternoon" and lesson.periodIndex < cutoff
                )
                if wrong:
                    violations += 1
                    affected.append(self._entity("teacher", teacher_id, teacher))
    ...
    def _teacher_is_available(teacher: Any, day: str, period: int) -> bool:
        ...
        for key, values in availability.items():
            ...
            if normalized == day:
                day_values = values
                break
        if day_values is not None and (period >= len(day_values) or not day_values[period]):
            return False
    ...
    def _room_stability(self):
        ...
        affected = [
            self._entity("class", class_id, self.class_map.get(class_id))
            for class_id, values in rooms.items()
            if len(values) > 1
        ]
        return violations, opportunities, affected
    ...
    def _suggestions(self, results: Iterable[ObjectiveResult]) -> list[Suggestion]:
        ...
        return [
            Suggestion(
                ...
                message_english=(
                    "This preference could not be fully satisfied; review the affected data."
                ),
