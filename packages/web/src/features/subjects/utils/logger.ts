/**
 * Debug logger utility for the Subjects feature
 *
...
function isDebugEnabled(): boolean {
  if (typeof window !== 'undefined') {
    return localStorage.getItem('debug:subjects') === 'true';
  }
...
function formatMessage(level: LogLevel, message: string, context?: LogContext): string {
  ...
  const prefix = `[${timestamp}] [subjects] [${level.toUpperCase()}]`;
...
/**
 * Logger instance for the Subjects feature
 *
 * Usage:
 * ```typescript
 ...
 * logger.info('Subject created', { id: 1, name: 'Mathematics' });
 * logger.warn('Subject already exists', { code: 'MATH101' });
 ...
 * ```typescript
 * useEffect(() => {
 *   componentLogger.mount('SubjectDataGrid');
 ...
 * Usage:
 * ```typescript
 * apiLogger.request('GET', '/api/subjects');
 * apiLogger.response('GET', '/api/subjects', 200, { count: 10 });
 * apiLogger.error('POST', '/api/subjects', error);
 * ```
 */
