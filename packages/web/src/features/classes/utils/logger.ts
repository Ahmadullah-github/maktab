/**
 * Debug logger utility for the Classes feature
 *
...
function isDebugEnabled(): boolean {
  if (typeof window !== 'undefined') {
    return localStorage.getItem('debug:classes') === 'true';
  }
...
function formatMessage(level: LogLevel, message: string, context?: LogContext): string {
  ...
  const prefix = `[${timestamp}] [classes] [${level.toUpperCase()}]`;
...
/**
 * Logger instance for the Classes feature
 *
 * Usage:
 * ```typescript
 ...
 * logger.info('Class created', { id: 1, name: 'Class 7-A' });
 * logger.warn('Room already assigned', { roomId: 5 });
 ...
 * ```typescript
 * useEffect(() => {
 *   componentLogger.mount('ClassDataGrid');
 *   return () => componentLogger.unmount('ClassDataGrid');
 * }, []);
 * ```
 */
 ...
 * Usage:
 * ```typescript
 * apiLogger.request('GET', '/api/classes');
 * apiLogger.response('GET', '/api/classes', 200, { count: 10 });
 * apiLogger.error('POST', '/api/classes', error);
 * ```
 */
