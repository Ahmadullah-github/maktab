...
function triggerBody(options: {
  hasMinistryValidationMode: boolean;
  hasEnableMinistryValidation: boolean;
  hasCustomCurriculumMode: boolean;
}): string {
  return `
    SELECT CASE WHEN
      ...
      ${options.hasMinistryValidationMode ? "NEW.ministryValidationMode IS NULL OR NEW.ministryValidationMode NOT IN ('off','warn','strict') OR" : ''}
    ...
    SELECT CASE WHEN
      ...
      NEW.ramadanModeEnabled NOT IN (0, 1) OR
      ${options.hasEnableMinistryValidation ? 'NEW.enableMinistryValidation NOT IN (0, 1) OR' : ''}
      ${options.hasCustomCurriculumMode ? 'NEW.customCurriculumMode NOT IN (0, 1) OR' : ''}
...
export class HardenPeriodConfiguration1784000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const legacyColumns = {
      hasMinistryValidationMode: Boolean(
        schoolConfigTable?.findColumnByName('ministryValidationMode')
      ),
      hasEnableMinistryValidation: Boolean(
        schoolConfigTable?.findColumnByName('enableMinistryValidation')
      ),
      hasCustomCurriculumMode: Boolean(
        schoolConfigTable?.findColumnByName('customCurriculumMode')
      ),
    };
    ...
    for (const row of rows) {
      ...
      if (legacyColumns.hasMinistryValidationMode) {
        assignments.push('ministryValidationMode = ?');
        parameters.push(ministryValidationMode);
      }
      assignments.push(
        'enablePrimary = ?',
        'enableMiddle = ?',
      ...
      if (legacyColumns.hasEnableMinistryValidation) {
        assignments.push('enableMinistryValidation = ?');
        parameters.push(Number(row.enableMinistryValidation) === 1 ? 1 : 0);
      }
      if (legacyColumns.hasCustomCurriculumMode) {
        assignments.push('customCurriculumMode = ?');
        parameters.push(Number(row.customCurriculumMode) === 1 ? 1 : 0);
      }
      assignments.push(
      ...
      await queryRunner.query(
        `UPDATE school_config SET ${assignments.join(', ')} WHERE id = ?`,
        parameters
      );
    }
    ...
    await queryRunner.query(
      `CREATE TRIGGER "TR_school_config_periods_insert" BEFORE INSERT ON "school_config" BEGIN ${triggerBody(legacyColumns)} END`
    );
    await queryRunner.query(
      `CREATE TRIGGER "TR_school_config_periods_update" BEFORE UPDATE ON "school_config" BEGIN ${triggerBody(legacyColumns)} END`
    );
  }
