...
from feedback.error_catalog import ErrorCode
from feedback.response_models import AffectedEntity, SolverErrorDetail, Suggestion
...
class PreSolveResult(BaseModel):
    ...
    can_proceed: bool = Field(..., description="Whether solving can proceed (no blocking errors)")
    ...
    analysis_time_ms: int = Field(..., ge=0, description="Time taken for analysis in milliseconds")
...
class PreSolveAnalyzer:
    ...
    def _check_teacher_capacity(self) -> list[SolverErrorDetail]:
        ...
        for assignment in self.data.fixedTeacherAssignments or []:
            if assignment.isFixed:
                fixed_by_pair.setdefault((assignment.classId, assignment.subjectId), []).append(
                    assignment
                )
        ...
        for cls in self.data.classes:
            ...
            if cls.singleTeacherMode and cls.classTeacherId:
                total_periods = sum(req.periodsPerWeek for req in cls.subjectRequirements.values())
        ...
        if not errors and flexible_demands:
            eligible_ids = {teacher.id for _, teachers in flexible_demands for teacher in teachers}
    ...
    def _check_consecutive_feasibility(self) -> list[SolverErrorDetail]:
        ...
        for class_group in self.data.classes:
            for subject_id, requirement in class_group.subjectRequirements.items():
                daily_limit = (
                    1 if not allow_consecutive else min(2, requirement.maxConsecutive or 2)
                )
    ...
    def _check_room_capacity(self) -> list[SolverErrorDetail]:
        ...
        fixed_room_ids = {cls.fixedRoomId for cls in self.data.classes if cls.fixedRoomId}
    ...
    def _check_room_type_requirements(self) -> list[SolverErrorDetail]:
        ...
        reported_missing_types: dict[str, list[str]] = {}  # room_type -> [subject_names]

        # Check each subject's room type requirement
        for subject in self.data.subjects:
            if subject.requiredRoomType and subject.requiredRoomType not in available_room_types:
        ...
        for room_type, subject_names in reported_missing_types.items():
            ...
            if first_subject:
                ...
                if len(subject_names) == 1:
                    ...
                    extra_subjects = (
                        f" and {len(subject_names) - 3} more" if len(subject_names) > 3 else ""
                    )
                    message_english = (
                ...
                error = SolverErrorDetail(
                    ...
                    context={
                        ...
                        "suggestion_farsi": (
                            f"لطفاً یک اتاق از نوع '{room_type}' اضافه کنید یا نوع اتاق "
                            "مورد نیاز مضامین را تغییر دهید"
                        ),
                        "suggestion_english": (
                            f"Please add a room of type '{room_type}' or change the required "
                            "room type for these subjects"
                        ),
                    },
                )
                errors.append(error)
        ...
        for cls in self.data.classes:
            for subject_id, req in cls.subjectRequirements.items():
                ...
                if req.minDaysPerWeek and req.minDaysPerWeek > num_days:
                    subject = next((s for s in self.data.subjects if s.id == subject_id), None)
                    subject_name = subject.name if subject else subject_id
