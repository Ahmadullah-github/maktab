...
import { limitRequestBodyBytes, validateRequest } from '../../middleware/validation.middleware';
...
export function createGenerateRoutes(dataSource: DataSource, cacheManager?: CacheManager): Router {
  ...
  router.post('/', limitRequestBodyBytes(64 * 1024), validateRequest(generateRequestSchema), (req, res) =>
    handleGenerate(dataSource, cacheManager, req, res)
  );
  router.get('/status', handleGetStatus);
  router.delete('/cancel', handleCancelGenerate);
  router.post('/analyze', limitRequestBodyBytes(64 * 1024), validateRequest(analyzeRequestSchema), (req, res) =>
