...
class RamadanConfig(BaseModel):
    ...
    period_duration: int = Field(
        default=35, ge=15, le=60, description="Duration of each period in minutes during Ramadan"
    )
    break_config: list[dict[str, Any]] | None = Field(
        default=None, description="Optional break period configurations for Ramadan"
    )
...
class RamadanModeHandler:
    ...
    def get_metadata(self) -> dict[str, Any]:
        ...
        return {
            ...
            "ramadanPeriodDuration": (self.config.period_duration if self.config.enabled else None),
        }
