...
class CheckpointManager:
    ...
    def __init__(self, checkpoint_dir: str = "./checkpoints"):
        ...
        log.info("CheckpointManager initialized", checkpoint_dir=str(self.checkpoint_dir))
    ...
    def save(
        ...
        try:
            ...
            log.error("Failed to save checkpoint", job_id=job_id, error=str(e))
    ...
    def load(self, job_id: str) -> Checkpoint | None:
        ...
        try:
            ...
            if not isinstance(checkpoint, Checkpoint):
                raise CheckpointCorruptError(f"Invalid checkpoint type: {type(checkpoint)}")
            ...
            log.error("Checkpoint file corrupted", job_id=job_id, error=str(e))
            raise CheckpointCorruptError(f"Checkpoint file corrupted: {e}") from e
        except Exception as e:
            log.error("Failed to load checkpoint", job_id=job_id, error=str(e))
            raise CheckpointCorruptError(f"Failed to load checkpoint: {e}") from e
    ...
    def delete(self, job_id: str) -> bool:
        ...
        try:
            ...
            log.error("Failed to delete checkpoint", job_id=job_id, error=str(e))
            return False

    def validate(self, checkpoint: Checkpoint, input_data: dict[str, Any]) -> bool:
        """
        ...
        if checkpoint.input_hash != current_hash:
            errors.append("Input data hash mismatch: checkpoint was created with different input")
        ...
        if errors:
            log.warning("Checkpoint validation failed", job_id=checkpoint.job_id, errors=errors)
            raise CheckpointValidationError(f"Checkpoint validation failed: {'; '.join(errors)}")

        log.info("Checkpoint validated successfully", job_id=checkpoint.job_id)
        return True
    ...
    def get_checkpoint_info(self, job_id: str) -> dict[str, Any] | None:
        """
