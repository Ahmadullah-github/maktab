...
class Logger {
  ...
  constructor() {
    ...
    this.logDirectory = process.env.MAKTAB_LOG_DIRECTORY;
    if (this.logDirectory) fs.mkdirSync(this.logDirectory, { recursive: true });
  }
  ...
  private output(level: LogLevel, entry: LogEntry): void {
    ...
    if (this.logDirectory) {
      this.rotateIfNeeded();
      fs.appendFileSync(path.join(this.logDirectory, 'local-api.log'), `${JSON.stringify(entry)}\n`, { encoding: 'utf8', mode: 0o600 });
    }
  ...
  private rotateIfNeeded(): void {
    ...
    const active = path.join(this.logDirectory, 'local-api.log');
    if (!fs.existsSync(active) || fs.statSync(active).size < this.maxBytes) return;
    fs.rmSync(path.join(this.logDirectory, `local-api.log.${this.maxFiles}`), { force: true });
    for (let index = this.maxFiles - 1; index >= 1; index -= 1) {
      const source = path.join(this.logDirectory, `local-api.log.${index}`);
      if (fs.existsSync(source)) fs.renameSync(source, path.join(this.logDirectory, `local-api.log.${index + 1}`));
    }
    fs.renameSync(active, path.join(this.logDirectory, 'local-api.log.1'));
  }
