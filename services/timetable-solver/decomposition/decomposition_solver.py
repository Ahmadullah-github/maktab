...
class DecompositionSolver:
    ...
    def _count_requests(self) -> int:
        ...
        for cls in self.data.classes:
            for _subject_id, req in cls.subjectRequirements.items():
    ...
    def _solve_with_clustering(self, **solver_kwargs) -> list[dict[str, Any]]:
        ...
        for i, cluster in enumerate(clusters):
            ...
            try:
                ...
                if not solution or (
                    isinstance(solution, list) and len(solution) > 0 and "error" in solution[0]
                ):
        ...
        if len(cluster_errors) == len(clusters):
            ...
            first_error = cluster_errors[0]["error"] if cluster_errors else "Unknown error"
            return [
                {
                    "error": first_error,
        ...
        if not merged or len(merged) == 0:
            first_error = cluster_errors[0]["error"] if cluster_errors else "No solution found"
            return [
                {
                    "error": first_error,
                    "status": "NO_SOLUTION",
        ...
        for grade_idx, (grade_level, grade_classes) in enumerate(grade_groups.items()):
            ...
            if sub_data.fixedLessons:
                sub_data.fixedLessons = [
                    lesson for lesson in sub_data.fixedLessons if lesson.classId in grade_class_ids
                ]
