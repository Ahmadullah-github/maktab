/**
 * Review Step Analysis Script
 * 
 * Runs all analyzers on the review-step component and generates
...
const STEP_NAME = 'review';
const STEP_FILE = 'packages/web/src/components/wizard/steps/review-step.tsx';
const OUTPUT_FILE = 'docs/review-audit.md';

// ============================================================================
// Main Analysis Function
// ============================================================================

async function analyzeReviewStep() {
  console.log('🔍 Starting Review Step Analysis...\n');
...
function generateAuditDocument(data: AuditData): string {
  ...
  let doc = `# Review Step - Deep Audit
...
This audit analyzes the Review wizard step across 9 dimensions: architecture, data flow, data passing, UI/UX, logic, styling, terminology, performance, and bilingual support.
...
1. **Data Summary Display**
   - Test school info summary rendering
   - Test periods summary rendering
   - Test subjects summary rendering
   - Test teachers summary rendering
   - Test classes summary rendering
   - Test rooms summary rendering
   - Test constraints summary rendering

2. **Data Validation**
   - Test completeness validation across all steps
   - Test data consistency checks
   - Test missing data detection
   - Test validation error display

3. **Edit Navigation**
   - Test "Edit" button navigation to specific steps
   - Test return to review after editing
   - Test data preservation during navigation
   - Test validation state after edits

4. **Export Functionality**
   - Test PDF export generation
   - Test export data completeness
   - Test export formatting (bilingual)
   - Test export error handling

5. **Submission Flow**
   - Test final submission validation
   - Test submission API call
   - Test submission success handling
...
1. **User Flow**
   - User reviews all data → identifies issue → edits step → returns to review → submits
   - User exports configuration → PDF generated with all data
   - User submits configuration → solver receives data → timetable generated

2. **Data Aggregation**
   - All wizard step data correctly aggregated in review
   - Changes in previous steps reflect immediately in review
   - Data counts and statistics are accurate
   - Missing data is clearly indicated

3. **Cross-Step Integration**
   - Review displays data from all 7 previous steps
   - Edit navigation works for all steps
   - Data consistency maintained across navigation
   - Validation status reflects all steps

4. **Submission Flow**
   - Complete validation → submission enabled
   - Incomplete data → submission disabled with clear messages
   - Successful submission → confirmation and next steps
   - Failed submission → error handling and retry options
