/**
 * Constraints Step Analysis Script
 * 
 * Runs all analyzers on the constraints-step component and generates
...
const STEP_NAME = 'constraints';
const STEP_FILE = 'packages/web/src/components/wizard/steps/constraints-step.tsx';
const OUTPUT_FILE = 'docs/constraints-audit.md';

// ============================================================================
// Main Analysis Function
// ============================================================================

async function analyzeConstraintsStep() {
  console.log('🔍 Starting Constraints Step Analysis...\n');
...
function generateAuditDocument(data: AuditData): string {
  ...
  let doc = `# Constraints Step - Deep Audit
...
This audit analyzes the Constraints wizard step across 9 dimensions: architecture, data flow, data passing, UI/UX, logic, styling, terminology, performance, and bilingual support.
...
1. **Constraint CRUD Operations**
   - Test adding new constraint
   - Test editing existing constraint
   - Test deleting constraint
   - Test constraint validation

2. **Constraint Type Management**
   - Test teacher-specific constraints
   - Test class-specific constraints
   - Test room-specific constraints
   - Test global constraints

3. **Constraint Validation**
   - Test constraint conflict detection
   - Test constraint priority handling
   - Test constraint applicability rules
   - Test constraint impact calculation

4. **Constraint Templates**
   - Test applying constraint templates
   - Test customizing template constraints
   - Test saving custom templates
   - Test template validation

5. **Constraint Preview**
   - Test constraint impact preview
   - Test affected entities display
   - Test constraint conflict warnings
   - Test constraint suggestions

### Integration Tests

1. **User Flow**
   - User adds constraint → configures details → saves
   - User edits constraint → changes parameters → saves
   - User deletes constraint → confirmation → removed from DB
   - User applies template → customizes → saves

2. **Data Persistence**
   - Changes persist when navigating away and back
   - Auto-save triggers after modifications
   - Data syncs with constraint store correctly
   - Constraint data available for solver

3. **Cross-Step Integration**
   - Constraints use teachers from Teachers step
   - Constraints use classes from Classes step
   - Constraints use rooms from Rooms step
   - Changes in previous steps reflect in Constraints step

4. **Validation Flow**
   - Validate all constraints → navigation enabled/disabled correctly
   - Fix validation errors → navigation enabled
   - Missing required data → clear error messages
   - Conflicting constraints → appropriate warnings
...
analyzeConstraintsStep().catch(error => {
