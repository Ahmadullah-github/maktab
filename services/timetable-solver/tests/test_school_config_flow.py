...
class SchoolConfigFlowTests(unittest.TestCase):
    ...
    def test_runtime_flags_are_part_of_the_solver_contract(self):
        ...
        self.assertNotIn("enableMinistryValidation", GlobalConfig.model_fields)
        self.assertNotIn("ministryValidationMode", GlobalConfig.model_fields)
        self.assertNotIn("customCurriculumMode", GlobalConfig.model_fields)

    def test_ministry_curriculum_and_validator_modules_are_absent(self):
        self.assertIsNone(importlib.util.find_spec("afghanistan.curriculum"))
        self.assertIsNone(importlib.util.find_spec("afghanistan.ministry_validator"))
    ...
    def test_solver_never_places_short_category_outside_its_boundary(self):
        ...
        alpha_lessons = [
            lesson for lesson in result["data"]["schedule"] if lesson["classId"] == "alpha-class"
        ]
