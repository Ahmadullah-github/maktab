...
class TestContainerizedSolver:
    ...
    def test_solver_health_check_passes(self, build_solver_image):
        ...
        result = subprocess.run(
            [
                ...
                build_solver_image,
                "-c",
                "import solver; import json; print('Solver health check passed')",
            ],
            capture_output=True,
            text=True,
            timeout=30,
    ...
    def test_solver_runs_correctly_in_container(
        self, build_solver_image, sample_input_data
    ):
        ...
        result = subprocess.run(
            ["docker", "run", "--rm", "-i", build_solver_image],
            input=input_json,
            capture_output=True,
            text=True,
            timeout=60,
        ...
        assert result.returncode in [
            0,
            1,
        ], f"Solver crashed unexpectedly: {result.stderr}"
        ...
        if result.returncode == 0 and result.stdout:
            try:
                ...
                assert isinstance(
                    output, list
                ), "Output should be a list of scheduled lessons"
    ...
    def test_solver_respects_environment_variables(
        self, build_solver_image, sample_input_data
    ):
        ...
        assert result.returncode in [
            0,
            1,
        ], f"Solver crashed with env vars: {result.stderr}"
    ...
    def test_solver_container_resource_limits(self, build_solver_image):
        ...
        result = subprocess.run(
            [
                ...
                build_solver_image,
                "-c",
                "import solver; print('Resource limits test passed')",
            ],
            capture_output=True,
            text=True,
            timeout=30,
    ...
    def test_docker_health_check_functionality(self, build_solver_image):
        ...
        try:
            ...
            ), f"Failed to inspect container: {inspect_result.stderr}"
            health_status = inspect_result.stdout.strip()

            # Health status should be "healthy"
            assert (
                health_status == "healthy"
            ), f"Container health check failed. Status: {health_status}"
