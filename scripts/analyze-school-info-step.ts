/**
 * School Info Step Analysis Script
 * 
 * Runs all analyzers on the school-info-step component and generates
...
const STEP_NAME = 'school-info';
const STEP_FILE = 'packages/web/src/components/wizard/steps/school-info-step.tsx';
const OUTPUT_FILE = 'docs/school-info-audit.md';

// ============================================================================
// Main Analysis Function
// ============================================================================

async function analyzeSchoolInfoStep() {
  console.log('🔍 Starting School Info Step Analysis...\n');
...
function generateAuditDocument(data: AuditData): string {
  ...
  let doc = `# School Info Step - Deep Audit
...
This audit analyzes the School Info wizard step across 9 dimensions: architecture, data flow, data passing, UI/UX, logic, styling, terminology, performance, and bilingual support.
...
1. **Validation Logic**
   - Test school name validation (min length, max length, required)
   - Test section toggle behavior (enablePrimary, enableMiddle, enableHigh)
   - Test schedule framework validation (daysPerWeek, periodsPerDay)

2. **State Management**
   - Test formData updates on input change
   - Test error state management
   - Test touched fields tracking

3. **Data Persistence**
   - Test onUpdate callback is called with correct data
   - Test data sync with parent component

### Integration Tests

1. **User Flow**
   - User enters school name → validation passes → Next button enabled
   - User toggles sections → data persists → navigates away and back → data retained
   - User changes schedule framework → updates propagate to next step

2. **Error Scenarios**
   - Empty school name → error message shown → Next button disabled
   - Invalid input → inline error displayed → error clears on valid input
...
function generateQuickWins(findings: AnalysisFinding[]): string {
  const quickWins = findings
    .filter(f => f.estimatedEffort === 'Small' && f.severity !== Severity.Low)
    .slice(0, 10);
  
  if (quickWins.length === 0) {
    return '\n✅ No quick wins identified (all issues require significant effort)';
  }
...
function printSummary(findings: AnalysisFinding[]) {
...
analyzeSchoolInfoStep().catch(error => {
