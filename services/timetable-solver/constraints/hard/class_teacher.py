...
class ClassTeacherMinLessonConstraint(HardConstraint):
    ...
    def apply(self, model: cp_model.CpModel, context: dict[str, Any]) -> None:
        ...
        for cls in data.classes:
            ...
            for r_idx in class_requests:
                ...
                if class_teacher_idx in allowed_teachers:
                    ...
                    is_class_teacher = model.NewBoolVar(f"is_class_teacher_{cls.id}_{r_idx}")
                    model.Add(teacher_vars[r_idx] == class_teacher_idx).OnlyEnforceIf(
                        is_class_teacher
                    )
                    model.Add(teacher_vars[r_idx] != class_teacher_idx).OnlyEnforceIf(
                        is_class_teacher.Not()
                    )
        ...
        if constraints_added > 0:
            log.info("Class teacher constraints applied", constraints_added=constraints_added)
