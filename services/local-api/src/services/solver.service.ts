...
export interface SolverRunEvidence {
  solverVersion: string;
  inputSha256: string;
  constraintCount: number;
  durationMs: number;
  outcome: 'success' | 'partial' | 'failed' | 'cancelled' | 'timeout' | 'crashed';
...
export class SolverService {
  private static readonly MAX_INPUT_BYTES = 25 * 1024 * 1024;
  ...
  private cachedReadiness: SolverReadiness | null = null;
  ...
  async preflight(timeoutMs = 10_000): Promise<SolverReadiness> {
    const { solverScript, pythonCommand, args } = this.resolveSolverPath();
    if (!fs.existsSync(solverScript)) {
      throw new Error(`Timetable solver is missing: ${solverScript}`);
    }

    const runProbe = (probe: '--version' | '--self-test'): Promise<string> =>
      new Promise((resolve, reject) => {
        const child = spawn(pythonCommand, [...args, probe], {
          cwd: path.dirname(solverScript),
          windowsHide: true,
        ...
        const timer = setTimeout(() => {
          child.kill('SIGKILL');
          reject(new Error(`Solver ${probe} timed out`));
        }, timeoutMs);
        ...
        child.once('exit', (code) => {
          clearTimeout(timer);
          if (code !== 0) reject(new Error(`Solver ${probe} failed (${code}): ${stderr.trim()}`));
          else resolve(stdout.trim());
        });
      });

    const version = await runProbe('--version');
    const selfTest = JSON.parse(await runProbe('--self-test')) as { status?: string; schema_version?: number };
    if (selfTest.status !== 'ok' || selfTest.schema_version !== 1) {
      throw new Error('Solver self-test returned an incompatible response');
    }
    const sha256 = crypto.createHash('sha256').update(await fs.promises.readFile(solverScript)).digest('hex');
    this.cachedReadiness = { version: version || 'unknown', sha256 };
    return this.cachedReadiness;
  }

  getSolverVersion(): string {
    return this.cachedReadiness?.version ?? 'unavailable';
  }
    ...
    if (this.currentTempFile) {
      try {
        if (fs.existsSync(this.currentTempFile)) {
          ...
          const directory = path.dirname(this.currentTempFile);
          if (path.basename(directory).startsWith('maktab-solver-')) fs.rmdirSync(directory);
          logger.debug('SolverService: Cleaned up temporary solver input');
        }
      } catch (err) {
  ...
  private terminateCurrentProcess(): void {
    if (this.currentProcess) {
      this.terminateProcessGracefully(this.currentProcess);
    }
  }
  ...
  private resetRunState(): void {
    ...
    if (this.currentProcess) this.terminateProcessGracefully(this.currentProcess);
  ...
  private resolveSolverPath(): {
    ...
    const fromCompiled = path.resolve(__dirname, '../../../../', 'timetable-solver');
    const fromSource = path.resolve(__dirname, '../../../', 'timetable-solver');
    const fromCwd = path.resolve(process.cwd(), 'services', 'timetable-solver');
    const fromCwdParent = path.resolve(process.cwd(), '..', 'timetable-solver');
    ...
    } else if (fs.existsSync(path.join(fromSource, SOLVER_SCRIPT_NAME))) {
      repoSolverDir = fromSource;
      logger.debug('SolverService: Using path from source location');
    ...
    if (envSolverPath) {
      ...
      pythonCommand = isPyScript ? this.findPythonCommand(solverDir) : solverScript;
      ...
      pythonCommand = this.findPythonCommand(solverDir);
      args = [solverScript];
  ...
  private findPythonCommand(solverDir: string): string {
    const virtualEnvironmentRoots = [
      ...
      path.resolve(solverDir, '..', '..', '.venv'),
      path.join(solverDir, '.venv'),
    ].filter((candidate): candidate is string => Boolean(candidate));

    for (const virtualEnvironmentRoot of virtualEnvironmentRoots) {
      const venvPython =
      ...
      if (fs.existsSync(venvPython)) {
        logger.info('SolverService: Using virtual environment Python', { path: venvPython });
        return venvPython;
      }
    }
  ...
  private writeToTempFile(data: any): string {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'maktab-solver-'));
    fs.chmodSync(tempDir, 0o700);
    const tempFile = path.join(tempDir, 'input.json');
    fs.writeFileSync(tempFile, JSON.stringify(data), { encoding: 'utf-8', mode: 0o600 });
  ...
  async runPreSolveAnalysis(data: any): Promise<PreSolveResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        this.throwIfCancellationRequested();
        const analysisInput = JSON.stringify(data);
        if (Buffer.byteLength(analysisInput, 'utf8') > SolverService.MAX_INPUT_BYTES) {
          const error = new Error('Generated solver input exceeds the 25 MiB safety limit') as SolverError;
          error.clientMessage = 'Timetable input is too large to analyze safely.';
          error.code = 'SOLVER_INPUT_TOO_LARGE';
          finalizeStandalone();
          return reject(error);
        }
        ...
        const timeout = setTimeout(() => {
          if (!finished) {
            this.terminateProcessGracefully(proc);
            finished = true;
        ...
        try {
          proc.stdin.write(analysisInput);
          proc.stdin.end();
        } catch (err) {
          logger.error(
  ...
  private executeSolver(data: any, timeoutMs: number): Promise<SolverResult> {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (dataSize > SolverService.MAX_INPUT_BYTES) {
          const error = new Error('Generated solver input exceeds the 25 MiB safety limit') as SolverError;
          error.clientMessage = 'Timetable input is too large to generate safely.';
          error.code = 'SOLVER_INPUT_TOO_LARGE';
          return reject(error);
        }
