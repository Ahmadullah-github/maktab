...
function toLicenseDeviceId(rawId) {
  return crypto
    .createHash('sha256')
    .update('maktab-license-device-v1\0')
    .update(rawId)
    .digest('base64url');
}

async function getLicenseDevice() {
  const id = toLicenseDeviceId(await getMachineId());
  const support = crypto.createHash('sha256').update(id).digest('hex').slice(0, 12).toUpperCase();
  return {
    id,
    supportCode: `${support.slice(0, 4)}-${support.slice(4, 8)}-${support.slice(8, 12)}`,
  };
}
