...
class AvoidTeacherGapsConstraint(SoftConstraint):
    ...
    def apply(
        self, model: cp_model.CpModel, context: Dict[str, Any]
    ) -> Optional[List[Any]]:
        ...
        for r_idx, req in enumerate(requests):
            # Check if r_idx is valid for the start_vars list
            if r_idx >= len(start_vars):
                continue
        ...
        for t_idx, req_indices in teacher_requests.items():
            ...
            for r_idx in req_indices:
                ...
                day_var = model.NewIntVar(
                    0, num_days - 1, f"teacher_gap_day_{t_idx}_{r_idx}"
                )
                periods_per_day_const = model.NewConstant(num_periods_per_day)
                model.AddDivisionEquality(
                    day_var, start_vars[r_idx], periods_per_day_const
                )
                day_vars[r_idx] = day_var

                # Period-in-day variable
                period_var = model.NewIntVar(
                    0, num_periods_per_day - 1, f"teacher_gap_period_{t_idx}_{r_idx}"
                )
                model.AddModuloEquality(
                    period_var, start_vars[r_idx], num_periods_per_day
                )
            ...
            for i in range(len(req_list)):
                for j in range(i + 1, len(req_list)):
                    ...
                    if r_i in teacher_vars and r_j in teacher_vars:
                        ...
                        model.Add(teacher_vars[r_i] == t_idx).OnlyEnforceIf(
                            is_i_assigned
                        )
                        model.Add(teacher_vars[r_i] != t_idx).OnlyEnforceIf(
                            is_i_assigned.Not()
                        )
                        model.Add(teacher_vars[r_j] == t_idx).OnlyEnforceIf(
                            is_j_assigned
                        )
                        model.Add(teacher_vars[r_j] != t_idx).OnlyEnforceIf(
                            is_j_assigned.Not()
                        ...
                        model.AddBoolAnd([is_i_assigned, is_j_assigned]).OnlyEnforceIf(
                            both_assigned
                        )
                        model.AddBoolOr(
                            [is_i_assigned.Not(), is_j_assigned.Not()]
                    ...
                    model.Add(day_vars[r_i] != day_vars[r_j]).OnlyEnforceIf(
                        same_day.Not()
                    )
                    ...
                    penalty_active = model.NewBoolVar(
                        f"teacher_gap_penalty_{t_idx}_{r_i}_{r_j}"
                    )

                    if isinstance(both_assigned, int) or (
                        hasattr(both_assigned, "Index") and both_assigned.Index() == -1
                    ):
                        # both_assigned is a constant (always true)
                        model.AddBoolAnd([same_day, has_gap]).OnlyEnforceIf(
                            penalty_active
                        )
                        model.AddBoolOr([same_day.Not(), has_gap.Not()]).OnlyEnforceIf(
                            penalty_active.Not()
                        )
                    else:
                        model.AddBoolAnd(
                            [both_assigned, same_day, has_gap]
                        ).OnlyEnforceIf(penalty_active)
                        model.AddBoolOr(
                            [both_assigned.Not(), same_day.Not(), has_gap.Not()]
                        ).OnlyEnforceIf(penalty_active.Not())
