/**
 * Subjects Step Analysis Script
 * 
 * Runs all analyzers on the subjects-step component and generates
...
const STEP_NAME = 'subjects';
const STEP_FILE = 'packages/web/src/components/wizard/steps/subjects-step.tsx';
const OUTPUT_FILE = 'docs/subjects-audit.md';

// ============================================================================
// Main Analysis Function
// ============================================================================

async function analyzeSubjectsStep() {
  console.log('🔍 Starting Subjects Step Analysis...\n');
...
function generateAuditDocument(data: AuditData): string {
  ...
  let doc = `# Subjects Step - Deep Audit
...
This audit analyzes the Subjects wizard step across 9 dimensions: architecture, data flow, data passing, UI/UX, logic, styling, terminology, performance, and bilingual support.
...
1. **Subject CRUD Operations**
   ...
   - Test subject validation (name, code, periods)

2. **Period Allocation**
   - Test period count validation per grade
   - Test period adjustment for subjects
   - Test total period calculation
   - Test period mismatch detection

3. **Grade Management**
   - Test grade filtering based on enabled sections
   - Test grade tab navigation
   - Test grade validation status
   - Test empty grade handling

4. **Curriculum Loading**
   - Test loading official curriculum for grade
   - Test inserting curriculum for all grades
   - Test clearing grade subjects
   - Test clearing all subjects

5. **Mode Switching**
   - Test Template Mode behavior
   - Test Saved Mode behavior
...
1. **User Flow**
   - User loads curriculum → subjects appear → periods match expected
   - User edits subject → changes persist → validation updates
   - User adds custom subject → saves → appears in list
   - User deletes subject → confirmation → removed from DB

2. **Data Persistence**
   - Changes persist when navigating away and back
   - Auto-save in Saved Mode works correctly
   - Manual save in Template Mode works correctly
   - Data syncs with subject store correctly

3. **Grade-Based Workflow**
   - Configure subjects for grade → switch grade → previous data persists
   - Validate all grades → navigation enabled/disabled correctly
   - Reset grade → subjects revert to defaults

4. **Cross-Step Integration**
   - Subjects available in Teachers step
   - Subjects available in Classes step
   - Subject changes reflect in dependent steps
...
analyzeSubjectsStep().catch(error => {
