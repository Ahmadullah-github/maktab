...
from strategies import BalancedStrategy, FastStrategy, ThoroughStrategy
...
def can_teach(
    teacher: dict,
    subject_id: str,
    class_group: Any | None = None,
    enforce_gender_separation: bool | None = False,
) -> bool:
    """Check if a teacher can teach a specific subject."""
    primary_subjects = teacher.get("primarySubjectIds", [])
    if subject_id not in primary_subjects and subject_id not in teacher.get(
        "allowedSubjectIds", []
    ):
        return False
...
class TimetableSolver:
    ...
    def __init__(self, timetable_data: dict | TimetableData):
        """
        ...
        try:
            ...
            log.error("Data validation failed", error=str(error))
            raise ValueError(f"Invalid timetable data structure: {error}") from error
    ...
    def _setup_constraint_registry(self) -> None:
        ...
        log.info("Constraint registry initialized", total_constraints=len(self.registry))

    def _normalize_days(self) -> None:
        """Normalize all Day values to canonical strings."""
        self.days = [
            d.value if isinstance(d, DayOfWeek) else str(d) for d in self.data.config.daysOfWeek
        ]
    ...
    def _periods_for_day(self, day_idx: int) -> int:
        ...
        if self.periods_per_day_map:
            return self.periods_per_day_map.get(self.days[day_idx], self.num_periods_per_day)
    ...
    def _prepare_data_maps(self) -> None:
        ...
        if cfg.categoryPeriodsPerDayMap:
            ...
            self.num_periods_per_day = max(
                max(day_map.values()) for day_map in cfg.categoryPeriodsPerDayMap.values()
            )
            self.periods_per_day_map = {
                day.value: max(
                    day_map.get(day, 0) for day_map in cfg.categoryPeriodsPerDayMap.values()
                )
            ...
            for subj_id, req in cls.subjectRequirements.items():
                ...
                try:
                    if self.data.preferences and (
                        self.data.preferences.allowConsecutivePeriodsForSameSubject is False
                    ):
                        min_c, max_c = 1, 1
                except Exception:
                    pass
    ...
    def _get_or_create_is_assigned(
        self, r_idx: int, t_idx: int | None = None, rm_idx: int | None = None
    ) -> cp_model.IntVar:
    ...
    def _create_variables(self) -> None:
        ...
        for r_idx, req in enumerate(self.requests):
            ...
            if domain_key in self.allowed_domains:
                ...
                if not allowed_teachers or not allowed_rooms:
                    ...
                    if not allowed_teachers:
                        ...
                        log.error(
                            f"Teachers with subject {s_id} in primarySubjectIds: "
                            f"{all_teachers_for_subject}"
                        )
                        ...
                        if domain_key in self.fixed_teacher_assignments:
                            ...
                            log.error(f"Fixed assignments for this class/subject: {fixed}")

                    # Build detailed error message
                    error_details = []
                    if not allowed_teachers:
                        error_details.append("Allowed teachers found: 0")
                    if not allowed_rooms:
                        required_room_type = subject.get("requiredRoomType", "any")
                        log.error(f"Subject requires room type: {required_room_type}")
                        log.error(f"Subject min capacity: {subject.get('minRoomCapacity', 0)}")
                        log.error(f"Class student count: {class_group.studentCount}")
                        error_details.append("Allowed rooms found: 0")
                        if required_room_type and required_room_type != "any":
                            error_details.append(f"requires room type: {required_room_type}")
                ...
                if not allowed_starts:
                    raise RuntimeError(f"No valid time slots for class '{c_id}', subject '{s_id}'")
            ...
            room_start_pairs = [pair for pair in room_start_pairs if pair[0] in allowed_start_set]
            ...
            self.model.AddAllowedAssignments([start_var, teacher_var], teacher_start_pairs)
    ...
    def _apply_hard_constraints(self) -> None:
        ...
        for _c_idx, intervals in self.class_intervals.items():
            if intervals:
                self.model.AddNoOverlap(intervals)

        # No-overlap for teachers
        for _t_idx, intervals in self.teacher_intervals.items():
            if intervals:
                self.model.AddNoOverlap(intervals)

        # No-overlap for rooms
        for _rm_idx, intervals in self.room_intervals.items():
            if intervals:
                self.model.AddNoOverlap(intervals)
    ...
    def _get_teacher_slot_occupancy(self, teacher_idx: int, slot: int):
        ...
        for r_idx, req in enumerate(self.requests):
            ...
            for start in self.request_allowed_starts[r_idx]:
                ...
                if at_start is None:
                    at_start = self.model.NewBoolVar(f"request_{r_idx}_starts_{start}")
    ...
    def _get_class_subject_slot_occupancy(self, class_id: str, subject_id: str | None, slot: int):
        ...
        occupied = self.model.NewBoolVar(f"class_{class_id}_{subject_id or 'all'}_occupied_{slot}")
        if covering:
            self.model.AddMaxEquality(occupied, covering)
        else:
            self.model.Add(occupied == 0)
    ...
    def _apply_schedule_shape_preference_constraints(self) -> list[Any]:
        ...
        for teacher_idx, _teacher in enumerate(self.data.teachers):
            available_day_loads = []
            ...
            if teacher_balance_weight > 0 and len(available_day_loads) > 1:
                ...
                target = self.model.NewIntVar(
                    0, self.num_periods_per_day, f"teacher_{teacher_idx}_daily_target"
                )
                ...
                for day_idx, load in enumerate(available_day_loads):
                    deviation = self.model.NewIntVar(
                        0,
                        self.num_periods_per_day,
                        f"teacher_{teacher_idx}_load_deviation_{day_idx}",
                    )
        ...
        difficult_distribution_weight = int(preferences.distributeDifficultSubjectsWeight * 100)
        difficult_subjects = {
        ...
        for teacher_idx, teacher in enumerate(self.data.teachers):
            ...
            for r_idx, req in enumerate(self.requests):
                if teacher_idx in self.request_allowed_teachers[r_idx]:
                    assignments.append(
                        req["length"] * self._get_or_create_is_assigned(r_idx, t_idx=teacher_idx)
                    )

            fixed_weekly = sum(
                1 for lesson in (self.data.fixedLessons or []) if teacher.id in lesson.teacherIds
            )
    ...
    def _apply_teacher_preference_constraints(self) -> list[Any]:
        ...
        if time_weight > 0:
            for r_idx, _req in enumerate(self.requests):
                rows = []
    ...
    def _apply_room_preference_constraints(self) -> list[Any]:
        ...
        desired_weight = int(getattr(prefs, "respectSubjectDesiredFeaturesWeight", 0.5) * 100)
        teacher_room_weight = int(getattr(prefs, "respectTeacherRoomPreferenceWeight", 0.5) * 100)
        ...
        for r_idx, req in enumerate(self.requests):
            ...
            if (
                ...
                self.model.Add(self.room_vars[r_idx] != home_room_idx).OnlyEnforceIf(outside_home)
            ...
            if desired_weight > 0 and desired_features:
                missing_by_room = [
                    len(desired_features - set(room.get("features") or []))
                ...
                if max_missing > 0 and any(missing_by_room[idx] for idx in allowed_rooms):
                    ...
                    self.model.AddElement(self.room_vars[r_idx], missing_by_room, missing_count)
            ...
            if teacher_room_weight > 0:
                ...
                for teacher_idx in allowed_teachers:
                    preferred_ids = {
                        str(value)
                        for value in (
                            self.data_dict["teachers"][teacher_idx].get("preferredRoomIds") or []
                        )
                    }
                    ...
                    for room_idx in allowed_rooms:
                        ...
                        penalty_value = int(bool(preferred_ids) and room_id not in preferred_ids)
                        preference_rows.append([teacher_idx, room_idx, penalty_value])
        ...
        room_change_weight = int(getattr(prefs, "minimizeRoomChangesWeight", 0.5) * 100)
        if room_change_weight > 0:
            fixed_rooms_by_class = collections.defaultdict(set)
            for lesson in self.data.fixedLessons or []:
                if lesson.roomId in self.room_map:
                    fixed_rooms_by_class[lesson.classId].add(self.room_map[lesson.roomId])
            ...
            for class_id, request_indices in self.class_to_requests.items():
                ...
                for room_idx in sorted(candidate_rooms - fixed_room_indices):
                    ...
                    if not assignments:
                        continue
                    room_used = self.model.NewBoolVar(f"class_{class_id}_uses_room_{room_idx}")
                ...
                if max_extra > 0:
                    ...
                    self.model.Add(extra_rooms == fixed_count + sum(used_vars) - 1)
    ...
    def solve(
        ...
        Args:
            ...
            optimization_level: 0=fastest, 1=balanced, 2=thorough
                (used when user_strategy is not provided).
        ...
        try:
            ...
            if self.num_requests > 0 and self.allowed_domains:
                ...
                total_allowed_rooms = sum(len(d["rooms"]) for d in self.allowed_domains.values())
            ...
            if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
                ...
                preference_revision = (
                    self.data.meta.optimizationPreferencesRevision if self.data.meta else None
                )
                enhanced_data.setdefault("metadata", {})["optimization"] = {
                    "preferencesRevision": preference_revision,
                    "effectivePreferences": (
                        self.data.preferences.model_dump() if self.data.preferences else {}
                    ),
                    "qualityScore": quality_score.model_dump(),
                }
                enhanced_data.setdefault("statistics", {})["qualityScore"] = quality_score.overall
                score = quality_score.overall if quality_score else "None"
                log.info(f"Calculated quality score: {score}")
...
def enhance_solution_with_metadata(solution: list[dict], data: TimetableData) -> dict[str, Any]:
    ...
    for cls in data.classes:
        class_info = {
            ...
            # Include for all classes, not just single-teacher mode.
    ...
    for teacher in data.teachers:
        teacher_info = {
            ...
            "classTeacherOf": [cls.id for cls in data.classes if cls.classTeacherId == teacher.id],
        }
