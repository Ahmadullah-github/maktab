...
export class ExportService {
  ...
  constructor(
    ...
    downloadBaseUrl: string = '/local-api/v1/export/download'
  ) {
  ...
  async exportSchedule(request: ExportRequest): Promise<ExportResponse> {
    return this.withTimeout(this.exportScheduleInternal(request), 'Export timed out');
  }

  private async exportScheduleInternal(request: ExportRequest): Promise<ExportResponse> {
    try {
      ...
      this.assertOutputSize(fileBuffer);
      ...
      await this.publishAtomically(partialPath, filePath, fileBuffer);
  ...
  private assertOutputSize(fileBuffer: Buffer): void {
    if (fileBuffer.length > ExportService.MAX_OUTPUT_BYTES) {
      throw new Error('Generated export exceeds the 100 MiB output limit');
    }
  }
  ...
  private withTimeout<T>(operation: Promise<T>, message: string): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error(message)), ExportService.TIMEOUT_MS);
      timer.unref();
      operation.then(
  ...
  async startBatchExport(request: ExportRequest): Promise<ExportJobResponse> {
    ...
    this.withTimeout(this.processBatchExport(jobId, request), 'Batch export timed out').catch((error) => {
