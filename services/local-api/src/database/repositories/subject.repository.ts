...
export class SubjectRepository extends BaseRepository<Subject> {
  ...
  private stringifySubjectJsonFields(input: SubjectInput): Partial<Subject> {
    ...
    return {
      ...
      curriculumItemId: normalized.curriculumItemId ?? null,
      grade,
      periodsPerWeek,
  ...
  async updateSubject(
    ...
    if (input.curriculumItemId !== undefined) {
      subject.curriculumItemId = input.curriculumItemId ?? null;
    }
  ...
  async findByCurriculumItemId(
    curriculumItemId: string,
    options?: RepositoryOptions
  ): Promise<ParsedSubject | null> {
    const subject = await this.getRepository(options?.manager).findOne({
      where: { curriculumItemId, isDeleted: false },
    });
    return subject ? this.parseSubjectJsonFields(subject) : null;
  }
