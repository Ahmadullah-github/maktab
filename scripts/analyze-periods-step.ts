/**
 * Periods Step Analysis Script
 * 
 * Runs all analyzers on the periods-step component and generates
...
const STEP_NAME = 'periods';
const STEP_FILE = 'packages/web/src/components/wizard/steps/periods-step.tsx';
const OUTPUT_FILE = 'docs/periods-audit.md';

// ============================================================================
// Main Analysis Function
// ============================================================================

async function analyzePeriodsStep() {
  console.log('🔍 Starting Periods Step Analysis...\n');
...
function generateAuditDocument(data: AuditData): string {
  ...
  let doc = `# Periods Step - Deep Audit
...
This audit analyzes the Periods wizard step across 9 dimensions: architecture, data flow, data passing, UI/UX, logic, styling, terminology, performance, and bilingual support.
...
1. **Break Period Configuration**
   - Test adding break after specific period
   - Test removing break (setting duration to 0)
   - Test updating break duration
   - Test break sorting after modifications

2. **Time Calculations**
   - Test addMinutes helper with various inputs
   - Test period time calculations with breaks
   - Test school day end time calculation
   - Test edge cases (midnight rollover, 24-hour format)

3. **Validation Logic**
   - Test break validation (excessive breaks, no breaks, optimal breaks)
   - Test schedule stats calculation
   - Test validation result generation

4. **State Management**
   - Test periodDuration state updates
   - Test schoolStartTime state updates
   - Test breakPeriods state updates
   - Test variable periods map updates
   - Test category periods map updates

5. **Preset Functionality**
   - Test morning break preset
   - Test lunch break preset
   - Test afternoon break preset
   - Test clear preset

### Integration Tests

1. **User Flow**
   - User sets school start time → periods calculate correctly
   - User changes period duration → schedule updates
   - User adds break → schedule recalculates → stats update
   - User applies preset → breaks configured → auto-save triggered

2. **Data Persistence**
   - Changes persist when navigating away and back
   - Auto-save triggers after break modifications
   - Data syncs with wizard store correctly

3. **Variable Periods Feature**
   - Enable variable periods → configuration UI appears
   - Set different periods per day → data persists
   - Disable variable periods → configuration hidden

4. **Category Periods Feature**
   - Enable category periods → category configs appear
   - Configure per-category schedules → data persists
   - Switch between variable and category modes
...
analyzePeriodsStep().catch(error => {
