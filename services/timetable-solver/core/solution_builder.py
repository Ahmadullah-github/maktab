...
def get_category_dari_name(category: str) -> str | None:
...
def normalize_break_periods(breaks: list[dict[str, Any]] | None) -> list[dict[str, int]]:
...
def serialize_category_periods(
    category_periods_map: dict[str, dict[Any, int]] | None,
    ...
    for category, day_map in category_periods_map.items():
        serialized[category] = {_day_to_string(day): periods for day, periods in day_map.items()}
...
def build_effective_break_periods_by_day(
    ...
    shared_breaks: list[dict[str, Any]] | None,
    break_periods_by_day: dict[Any, list[dict[str, Any]]] | None,
    ...
    for day in days:
        ...
        resolved_breaks = (
            normalized_overrides[day] if day in normalized_overrides else shared_breaks
        )
...
def get_periods_for_class_day(
    ...
    if cfg.categoryPeriodsPerDayMap:
        if not class_category:
            raise ValueError("Class category is required when categoryPeriodsPerDayMap is enabled")

        category_map = cfg.categoryPeriodsPerDayMap.get(class_category)
        if not category_map:
            raise ValueError(f"Class category '{class_category}' has no period configuration")
...
class SolutionBuilder:
    ...
    def build_solution(
        ...
        solution.sort(key=lambda x: (self.day_map.get(x["day"], 0), x["periodIndex"], x["classId"]))
