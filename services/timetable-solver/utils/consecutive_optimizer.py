...
class ConsecutiveOptimizer:
    ...
    @staticmethod
    def optimize_gap_constraints(
        lessons: list[tuple[int, int, int, int]], model, start_vars: list
    ) -> int:
        """
        ...
        log.debug(
            f"Optimized gap constraints: {constraints_added} constraints for "
            f"{len(lessons)} lessons (avoided "
            f"{len(lessons) * (len(lessons) - 1) // 2 - constraints_added} "
            "redundant checks)"
        )
    ...
    @staticmethod
    def group_lessons_by_day(
        lessons: list[tuple[int, int, int, int]], num_periods_per_day: int
    ) -> dict[int, list]:
        """
    ...
    @staticmethod
    def check_consecutive_feasibility(
        lessons: list[tuple[int, int, int, int]], max_consecutive: int, num_periods_per_day: int
    ) -> tuple[bool, str]:
        """
        ...
        Returns:
            (is_feasible, reason)
        """
        total_periods = sum(lesson[3] for lesson in lessons)
    ...
    @staticmethod
    def optimize_consecutive_constraints(
        class_subject_lessons: dict, consecutive_settings: dict, num_periods_per_day: int
    ) -> dict:
        """
        ...
        """
        optimized = {}
        stats = {"total_groups": 0, "skipped_infeasible": 0, "optimized_groups": 0}
        ...
        for (class_id, subject_id), lessons in class_subject_lessons.items():
            ...
            if consecutive_periods > 1:
                ...
                if not is_feasible:
                    log.debug(
                        "Skipping infeasible consecutive constraint for "
                        f"{class_id}/{subject_id}: {reason}"
                    )
        ...
        log.info(
            "Consecutive constraint optimization complete",
