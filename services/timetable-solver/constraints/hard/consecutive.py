        ...
        for class_id, subjects in class_subject_day_lessons.items():
            ...
            for subject_id, days in subjects.items():
                ...
                allow_consecutive = bool(
                    getattr(
                        getattr(data, "preferences", None),
                        "allowConsecutivePeriodsForSameSubject",
                        True,
                    )
                )
                consecutive_setting = (
                    (getattr(subject_req, "maxConsecutive", None) or 2) if allow_consecutive else 1
                )
    ...
    def _apply_day_constraints(
        ...
        model.Add(
            ...
            == sum(
                ...
                for lesson, is_on_day in zip(lessons, lessons_on_day_bools, strict=False)
            )
        )
    ...
    def _apply_adjacency_constraints(
        ...
        for i, lesson_i in enumerate(lessons):
            for j, lesson_j in enumerate(lessons[i + 1 :], start=i + 1):
                ...
                model.AddBoolOr([is_i_on_day.Not(), is_j_on_day.Not()]).OnlyEnforceIf(
                    both_on_day.Not()
                )
