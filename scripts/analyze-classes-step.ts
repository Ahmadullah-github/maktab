/**
 * Classes Step Analysis Script
 * 
 * Runs all analyzers on the classes-step component and generates
...
const STEP_NAME = 'classes';
const STEP_FILE = 'packages/web/src/components/wizard/steps/classes-step.tsx';
const OUTPUT_FILE = 'docs/classes-audit.md';

// ============================================================================
// Main Analysis Function
// ============================================================================

async function analyzeClassesStep() {
  console.log('🔍 Starting Classes Step Analysis...\n');
...
function generateAuditDocument(data: AuditData): string {
  ...
  let doc = `# Classes Step - Deep Audit
...
This audit analyzes the Classes wizard step across 9 dimensions: architecture, data flow, data passing, UI/UX, logic, styling, terminology, performance, and bilingual support.
...
1. **Class CRUD Operations**
   ...
   - Test deleting class
   - Test class validation (name, grade, student count)

2. **Teacher Assignment**
   - Test assigning teacher to class for subject
   - Test preventing duplicate assignments
   - Test teacher workload calculation
   - Test assignment validation

3. **Subject Assignment**
   - Test assigning subjects to class
   - Test subject period allocation
   - Test total period validation
...
5. **Single-Teacher Mode**
   - Test single-teacher mode detection
   - Test automatic teacher assignment
   - Test subject assignment in single-teacher mode
   - Test mode switching

### Integration Tests

1. **User Flow**
   - User adds class → assigns subjects → assigns teachers → saves
   - User edits class → changes assignments → saves
   - User deletes class → confirmation → removed from DB
   - User switches grades → previous data persists

2. **Data Persistence**
   - Changes persist when navigating away and back
   - Auto-save triggers after modifications
   - Data syncs with class store correctly
   - Teacher assignments persist correctly

3. **Cross-Step Integration**
   - Classes use subjects from Subjects step
   - Classes use teachers from Teachers step
   - Class data available in subsequent steps
   - Changes in previous steps reflect in Classes step
...
analyzeClassesStep().catch(error => {
