...
class DomainFilter:
    ...
    def _build_room_subject_compatibility(self) -> dict[tuple[int, int], bool]:
        ...
        for r_idx, room in enumerate(self.data.rooms):
            for s_idx, subject in enumerate(self.data.subjects):
                ...
                req_features = {
                    feature.strip().lower()
                    for feature in (subject.requiredFeatures or [])
                    if feature.strip()
                }
                room_features = {
                    feature.strip().lower() for feature in (room.features or []) if feature.strip()
                }
    ...
    def filter_teachers_for_request(
        self, subject_idx: int, allowed_teachers: list[int]
    ) -> list[int]:
        """
    ...
    def filter_rooms_for_request(
        self, subject_idx: int, class_size: int, allowed_rooms: list[int]
    ) -> list[int]:
        """
    ...
    def get_stats(self) -> dict:
        ...
        teacher_reduction = 1.0 - (
            len(self.teacher_subject_compat) / total_teacher_subject_combinations
        )
