...
import { limitRequestBodyBytes, positiveIntegerParam, textParam } from '../middleware/validation.middleware';
...
import { LocalApiError } from '../errors/localApiError';
...
export function createExportRoutes(dataSource: DataSource, cacheManager?: CacheManager): Router {
  ...
  router.post('/schedule/:id', limitRequestBodyBytes(64 * 1024), async (req, res) => {
    try {
      ...
      if (/timed out/i.test(message)) {
        throw new LocalApiError(504, 'EXPORT_TIMEOUT', 'Export timed out', undefined, true);
      }
      if (/100 MiB|too large|maximum 50/i.test(message)) {
        throw new LocalApiError(413, 'EXPORT_TOO_LARGE', message);
      }
      throw new LocalApiError(500, 'EXPORT_FAILED', 'Export failed');
    }
  });
