...
def main():
    ...
    parser.add_argument("--version", action="store_true", help="Print solver version and exit")
    parser.add_argument(
        "--self-test", action="store_true", help="Run a bounded executable self-test"
    )
    parser.add_argument(
        "--analyze-only",
        action="store_true",
    ...
    parser.add_argument("--input-file", type=str, help="Read input from file instead of stdin")
    args = parser.parse_args()

    if args.version:
        try:
            solver_version = version("maktab-timetable-solver")
        except PackageNotFoundError:
            solver_version = "development"
        print(solver_version)
        return
    if args.self_test:
        print(json.dumps({"status": "ok", "schema_version": 1}))
        return
    ...
    try:
        ...
        # Default logs deliberately contain counts only. Entity names and the
        # solver payload are customer data and belong only in explicit exports.
        ...
        if isinstance(solution, dict) and "status" in solution:
            ...
            if status in ("success", "partial"):
                log.info(
                    ...
                    status=status,
                    data_keys=(
                        list(solution.get("data", {}).keys()) if solution.get("data") else []
                    ),
                ...
                error_msg = (
                    errors[0].get("message_english", "Unknown error") if errors else "No solution"
                )
            ...
            if solution and len(solution) > 0:
                ...
                solution[0].get("status", "SOLVER_ERROR")
                cluster_errors = solution[0].get("cluster_errors", [])

            log.error("=== SOLVER FAILED ===", error=error_msg)

            # Parse the error message to extract useful information
            # Common pattern: no valid resources for a named class and subject.
            import re
            ...
            if "No valid teachers or rooms" in error_msg:
                ...
                for ce in cluster_errors:
                    ...
                    if "Allowed rooms found: 0" in ce_error or "room type:" in ce_error.lower():
                        has_room_issue = True
                        room_type_match = re.search(r"requires room type: ([^\n|]+)", ce_error)
                        if room_type_match:
                ...
                if (
                    ...
                    if room_type_match:
                        room_type = room_type_match.group(1).strip()
                    message_farsi = (
                        "اتاق مناسب برای مضمون یافت نشد. این مضمون به اتاق نوع "
                        f"'{room_type}' نیاز دارد"
                    )
                    suggestion_farsi = (
                        f"لطفاً یک اتاق از نوع '{room_type}' اضافه کنید یا نوع اتاق مورد "
                        "نیاز مضمون را تغییر دهید"
                    )
                    ...
                    message_farsi = "منابع کافی (استاد یا اتاق) برای این صنف و مضمون یافت نشد"
                ...
                "errors": [
                    {
                        ...
                        "message_english": (error_msg[:500] if len(error_msg) > 500 else error_msg),
        ...
        print(
            json.dumps({"error": error_msg, "status": "INVALID_INPUT", "details": str(e)}),
            file=sys.stderr,
        )
        sys.exit(1)
        ...
        print(
            json.dumps({"error": error_msg, "status": "VALIDATION_ERROR", "details": str(e)}),
            file=sys.stderr,
        )
        sys.exit(1)
        ...
        print("=== END TRACEBACK ===", file=sys.stderr)
        print(
            json.dumps({"error": error_msg, "status": "SOLVER_CRASH", "details": str(e)}),
            file=sys.stderr,
        )
        sys.exit(1)
        ...
        print(
            json.dumps({"error": "Solver interrupted by user", "status": "INTERRUPTED"}),
            file=sys.stderr,
        )
        sys.exit(1)
