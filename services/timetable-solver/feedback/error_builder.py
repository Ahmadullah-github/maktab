...
def _validate_context(error_code: ErrorCode, context: dict[str, Any]) -> None:
    ...
    missing_keys = [key for key in definition.required_context_keys if key not in context]

    if missing_keys:
        raise ValueError(f"Missing required context keys for {error_code.value}: {missing_keys}")
