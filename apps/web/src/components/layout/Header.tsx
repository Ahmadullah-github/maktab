...
export const Header = () => {
  ...
  const platformStatus = usePlatformSessionStore((state) => state.status);
  ...
  const displayName = session?.account.display_name || session?.account.username;
  const roleNames = activeMembership?.roles.map((role) => role.name).join(' • ');
      ...
      <div className="flex items-center gap-4 sm:gap-6">
        ...
        <div className="hidden md:flex items-center gap-3 bg-muted/30 px-3 py-1.5 rounded-full border border-border/50">
          <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider text-natural">
            {runtime.capabilities.platform && platformStatus === 'authenticated'
        ...
        {/* Status Indicators */}
        {runtime.capabilities.platform && <div className="flex items-center gap-2">
          <div
        ...
        >
          {runtime.capabilities.platform && <Button variant="ghost" size="icon" className="text-muted-foreground">
          ...
          {runtime.capabilities.platform && (platformStatus === 'authenticated' ? (
            <div className="flex items-center gap-3">
              <div className={`${isRTL ? 'text-start' : 'text-end'} hidden sm:block`}>
                <div className="text-sm font-medium leading-none text-natural">{displayName}</div>
                <div className="text-xs text-muted-foreground mt-1 text-natural">
                  {roleNames || t('header.administrator')}
                </div>
              </div>
